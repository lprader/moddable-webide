...
export const onInitialize: OnInitialize = async ({
	...
	if (!(await effects.Storage.hasProject('helloworld'))) {
		await actions.Storage.openProject('helloworld');
		await actions.importGist('b3ac430418909bf101a3a4e76c69d23a');
	}
	...
	/*else if (await effects.Storage.hasProject('time-date-weather')) {
		...
		await actions.Storage.openProject('colored-squares');
